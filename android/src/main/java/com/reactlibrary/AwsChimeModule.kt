...
class AwsChimeModule(
    ...
    fun startMeeting(meetingInfo: ReadableMap, attendeeInfo: ReadableMap) {
        ...
        currentActivity?.let { activity ->
            ...
            try {
                ...
                if (meetingSession != null) {
                    Companion.meetingSession = meetingSession
                    ...
                    eventEmitter.sendReactNativeEvent(RNEventEmitter.RN_EVENT_ERROR, "Failed to create meeting session")
                }
            } catch (exception: Exception) {
                logger.error(TAG, "Error starting the meeting session: ${exception.localizedMessage}")
                eventEmitter.sendReactNativeEvent(RNEventEmitter.RN_EVENT_ERROR, "Error starting the meeting session: ${exception.localizedMessage}")
                return
            }
        }
    }
    ...
    private fun createSessionConfiguration(meetingInfo: ReadableMap, attendeeInfo: ReadableMap): MeetingSessionConfiguration? {
        return try {
            ...
            eventEmitter.sendReactNativeEvent(RNEventEmitter.RN_EVENT_ERROR, "Error creating session configuration: ${exception.localizedMessage}")
            null
        }
    }
        ...
        return when (requestCode) {
            WEBRTC_PERMISSION_REQUEST_CODE -> {
                ...
                if (isMissingPermission) {
                    eventEmitter.sendReactNativeEvent(RNEventEmitter.RN_EVENT_ERROR, "Unable to start meeting as permissions are not granted")
                    false
                } else {
                    startAudioVideo()
                    true
